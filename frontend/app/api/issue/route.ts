...
function attributeToValue(
  ...
  switch (type) {
    ...
    case "age": {
      ...
      if (!dob)
        throw new Error("age credential requires attributes.date_of_birth");
    ...
    case "income": {
      ...
      if (!Number.isFinite(income))
        throw new Error("income credential requires attributes.income");
    ...
    case "jurisdiction": {
      ...
      if (!Number.isFinite(country))
        throw new Error(
          "jurisdiction credential requires attributes.country_code",
        );
    ...
    case "funds": {
      ...
      if (!Number.isFinite(balance))
        throw new Error("funds credential requires attributes.balance");
    ...
    case "accreditation": {
      ...
      if (!Number.isFinite(netWorth))
        throw new Error(
          "accreditation credential requires attributes.net_worth",
        );
...
function signCommitment(commitment: string): {
  ...
  const sig = secp256k1.sign(be32(BigInt(commitment)), DEMO_SK, {
    prehash: false,
  });
